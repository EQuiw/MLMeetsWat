...
class TreeExtractor(object):
    ...
    Change Log: self.all_leaves instead of all_leaves. See method extract.
    """
    ...
    def extract(self, incomplete_queries=False):
        ...
        while query_pool:
            ...
            # query
            if leaf_id in self.all_leaves and pred_valid(query, self.all_leaves[leaf_id]):
                ...
                # previous path
                if leaf_id in self.all_leaves:
                    logging.log(DEBUG, "TWO DIFFERENT LEAVES HAVE SAME ID!!!")
                    self.all_leaves[leaf_id].append(predicates+query_pred)
                else:
                    self.all_leaves[leaf_id] = [predicates+query_pred]
    ...
    def get_leaves_after_exception(self):
        """ Necessary change so that if boundary throws exception. We can still extract the leaves obtained so far"""
        return self.all_leaves
            ...
            if not incomplete_queries or incomplete_id != leaf_id:
                if isinstance(feature, ContFeature):
                    ...
                    if thresholds:
                        ...
                        # test for range a,b if needs to be included
                        # if query value is smaller than a or larger than b
                        ...
                        if query[feature.name] <= thresholds[-1]:
                            # pred = GT(feature, thresholds[-1], (feature.max_val-thresholds[-1])/2.0)
                            pred = GT(feature, thresholds[-1], self.epsilon)
                            new_query = pred.apply(query)
                            queries += [(new_query, [pred])]
