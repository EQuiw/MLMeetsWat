...
typedef struct _it_transform2D_ {
  ...
  void (* get_output_size)(struct _it_transform2D_ *transform, idx_t *width, idx_t *height);

  /* fix the length of the vectors on which to apply the transform */
  void (* set_size)(struct _it_transform2D_ *transform, idx_t width, idx_t height);
  ...
  void (* get_size)(struct _it_transform2D_ *transform, idx_t *width, idx_t *height);
...
static inline void __it_transform2D_get_size(it_transform2D_t *t, idx_t *w, idx_t *h)
{
  t->get_size(t, w, h);
}
